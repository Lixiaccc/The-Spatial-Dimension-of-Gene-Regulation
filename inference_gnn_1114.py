...
# -----------------------
# Build PairedSCDataset (same tokenization as finetune)
# -----------------------
...
# -----------------------
# EpiFoundation forward (token-space) -> gene-aligned (G=3000)
# -----------------------
@torch.no_grad()
def epifoundation_collect_gene_aligned_from_paired(
    ...
    for bi, batch in enumerate(loader, start=1):
        ...
        rna_ids    = batch["rna_ids"].to(device)     # (B, L=rna_max_len)
...
# -----------------------
# GNN refiner modules (must match training)
# -----------------------
...
def main():
    ...
    ap.add_argument("--rna_key", required=True)   # X_binned
    ap.add_argument("--atac_key", required=True)  # X
    ...
    print("="*70)
    truth_gene, gene_names, rna_obs, rna_obs_names = load_truth_gene_matrix(args.rna_test, layer_key=args.rna_key)
    ...
    print("\n" + "="*70)
    print("STEP 2: Build PairedSCDataset for test (same as finetune)")
    print("="*70)
    ...
    # coords from ATAC obs (for graph + saving)
    ...
    print("\n" + "="*70)
    print("STEP 3: Run EpiFoundation baseline (token-space -> gene-aligned)")
    print("="*70)
    model = EpiFoundation(
    ...
    print("\n" + "="*70)
    print("STEP 4: Build kNN graph from (x,y)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("STEP 5: Run GNN refiner (same representation as training)")
    print("="*70)
    gnn_ckpt = torch.load(args.gnn_ckpt, map_location=device)

    P = int(gnn_ckpt.get("proj_dim", 256))
    alpha = float(gnn_ckpt.get("alpha", 1.0))
    gnn_layers = int(gnn_ckpt["config"].get("gnn_layers", 3)) if "config" in gnn_ckpt else 3
    gnn_hidden = int(gnn_ckpt["config"].get("gnn_hidden", 256)) if "config" in gnn_ckpt else 256
    dropout = float(gnn_ckpt["config"].get("dropout", 0.0)) if "config" in gnn_ckpt else 0.0

    D = cell_emb.shape[1]
    gene_enc = GenePredEncoder(G=G, out_dim=P, hidden=max(512, P*2), dropout=dropout).to(device)
    gnn = ResidualGNN(dim=D + P, hidden_dim=gnn_hidden, n_layers=gnn_layers, dropout=dropout).to(device)
    head = GeneDeltaHead(in_dim=D + P, hidden_dim=gnn_hidden, out_dim=G, dropout=dropout).to(device)

    if "gene_enc" in gnn_ckpt:
        gene_enc.load_state_dict(gnn_ckpt["gene_enc"])
    elif "base_enc" in gnn_ckpt:
        gene_enc.load_state_dict(gnn_ckpt["base_enc"])
    else:
        raise KeyError("GNN ckpt missing gene encoder weights (expected 'gene_enc' or 'base_enc').")
    ...
    out.uns["test_eval"] = {
        ...
        "note": "Baseline matches finetune tokenization distribution via PairedSCDataset; gene-aligned via id2pos mapping."
    }
