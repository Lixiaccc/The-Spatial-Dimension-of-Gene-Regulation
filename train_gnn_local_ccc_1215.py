...
def apply_edge_dropout_with_feat(edge_index_t: torch.Tensor, edge_feat_t: torch.Tensor, drop_p: float, training: bool):
    """
    Edge dropout but keeps edge_feat aligned.
    Returns: edge_index_use, edge_feat_use
    """
    if (not training) or drop_p <= 0.0:
        return edge_index_t, edge_feat_t
    ...
    return edge_index_t[:, keep], edge_feat_t[keep]


# ============================================================
# 5.5) CCC parsing + kNN-edge CCC features
# ============================================================
...
def parse_ccc_out_map(obs: pd.DataFrame):
    """
    Build out_map[i] = set(indices j) that i sends to, using pair_key lists.
    Uses obs['pair_key'] for unique ID (species-prefixed).
    """
    ...
    if "sigccc_out_pair_keys" not in obs.columns:
        print("[WARN] No sigccc_out_pair_keys in obs; CCC features will be all zeros.")
    ...
    for i in range(len(obs)):
        ...
        if out_str == "" or out_str.lower() == "nan":
            continue
        # semicolon-delimited list of pair_keys
...
def build_ccc_edge_feat_for_knn(edge_index_np: np.ndarray, out_map: list):
    """
    For each kNN directed edge (i -> j) build:
      ccc_out_ij = 1 if i sends to j
      ccc_in_ij  = 1 if j sends to i  (i.e. reverse out)
    Returns: (E, 2) float32
    """
    src = edge_index_np[0]
    dst = edge_index_np[1]
...
# ============================================================
# 6) GNN refiner (gene-level) with CCC-aware attention
# ============================================================

class EdgeAttention(nn.Module):
    """
    Edge feature -> attention weight in [0,1].
...
class ResidualGNNLayer(nn.Module):
    def __init__(self, dim, hidden_dim, edge_dim=3, dropout=0.0):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(dim * 2 + 1, hidden_dim),  # x, agg, degree
            nn.ReLU(),
            nn.Dropout(dropout),
...
def save_curves(out_png_loss: str, out_png_pcc: str, history: dict):
    ...
    plt.title("GNN gene refiner loss (CCC-attn)")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("epoch")
    plt.ylabel("PCC")
    plt.title("GNN gene refiner validation PCC (CCC-attn)")
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    # ---- CCC edge features on kNN edges (IN+OUT) ----
    tr_out_map, _ = parse_ccc_out_map(tr_obs)
    va_out_map, _ = parse_ccc_out_map(va_obs)

    tr_ccc_e = build_ccc_edge_feat_for_knn(tr_edge_index_np, tr_out_map)  # (E,2)
    va_ccc_e = build_ccc_edge_feat_for_knn(va_edge_index_np, va_out_map)
    ...
    tr_ccc_t = torch.from_numpy(tr_ccc_e).float().to(device)
    va_ccc_t = torch.from_numpy(va_ccc_e).float().to(device)

    tr_edge_feat_t = torch.cat([tr_ed_t.unsqueeze(-1), tr_ccc_t], dim=1)  # (E,3)
    va_edge_feat_t = torch.cat([va_ed_t.unsqueeze(-1), va_ccc_t], dim=1)

    print(f"[INFO] Edge feat dim = {tr_edge_feat_t.shape[1]} (dist + ccc_out + ccc_in)")
    print(f"[INFO] CCC hit rate (train): out={(tr_ccc_e[:,0].mean()):.4f}, in={(tr_ccc_e[:,1].mean()):.4f}")
    print(f"[INFO] CCC hit rate (val):   out={(va_ccc_e[:,0].mean()):.4f}, in={(va_ccc_e[:,1].mean()):.4f}")

    print("\n" + "="*70)
    print("STEP E: TRAIN GNN GENE REFINER (CCC-attn)")
    print("="*70)
    ...
    def refine(emb_t, base_gene_t, ei_t, edge_feat_t, deg_t, training=False, return_delta=False):
        # edge dropout (keeps edge_feat aligned)
        ei_use, ef_use = apply_edge_dropout_with_feat(ei_t, edge_feat_t, args.edge_dropout, training=training)

        z = gene_enc(base_gene_t)          # (N, P)
        x = torch.cat([emb_t, z], dim=1)   # (N, D+P)

        x2 = gnn(x, ei_use, ef_use, deg_t)
        delta = head(x2)                   # (N, G)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pred_tr, delta_tr = refine(
            tr_emb_t, tr_base_t, tr_ei_t, tr_edge_feat_t, tr_deg,
            training=True, return_delta=True
        )
        ...
        with torch.no_grad():
            pred_va = refine(
                va_emb_t, va_base_t, va_ei_t, va_edge_feat_t, va_deg,
                training=False
            )
    ...
    gene_enc.eval(); gnn.eval(); head.eval()
    with torch.no_grad():
        pred_va_best = refine(va_emb_t, va_base_t, va_ei_t, va_edge_feat_t, va_deg, training=False).detach().cpu().numpy()
        pred_tr_best = refine(tr_emb_t, tr_base_t, tr_ei_t, tr_edge_feat_t, tr_deg, training=False).detach().cpu().numpy()
    ...
    out.uns["eval_info"] = {
        ...
        "gnn_edge_features": "concat(dist, ccc_out(i->j), ccc_in(j->i))",
        "alpha": float(alpha),
        "proj_dim": int(P),
